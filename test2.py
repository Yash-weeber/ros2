...
def resample_trajectory():
    ...
    # 2. Find Min/Max to understand the shape
    ...
    # 3. Define the "SAFE ZONE" on the table (Relative to Robot)
    # We want the pattern to happen directly in front of UR1
    SAFE_MIN_X = 0.05  # Was -0.25
    SAFE_MAX_X = 0.35  # Was  0.25
    SAFE_MIN_Y = 0.35  # Was  0.20
    SAFE_MAX_Y = 0.65  # Was  0.50

    safe_width = SAFE_MAX_X - SAFE_MIN_X
    safe_height = SAFE_MAX_Y - SAFE_MIN_Y

    # Calculate scaling factor to preserve the aspect ratio of the drawing
    scale = min(safe_width / orig_width, safe_height / orig_height)

    # 4. Apply Scaling and Translation
    print(f"Applying Scale Factor: {scale:.4f}")
    ...
    with open(output_csv, 'w', newline='') as f:
        ...
        for p in original_points:
            # Shift to 0,0 -> Scale -> Shift to Safe Zone Center
