...
class DualMopControlSystem(Node):
    ...
    def __init__(self):
        ...
        rest_qs_r2 = [-37.32, -51.07, 99.88, -245.13, -66.17, 268.34]
        ...
        self.ur1_const_z = 0.759  # Robot 1 mop tip height above floor (metres)
    ...
    def run_robot1_phase(self):
        ...
        if choice == '1':
            ...
            if not os.path.exists(csv_path):
                ...
                if points:
                    # 1) Make IK points continuous vs current joints (avoid wrist flips)
                    start_q = self.current_joints_1
                    if start_q is None:
                        print("Robot 1 joint state unavailable; aborting CSV execution.")
                        return
                    ...
                    # 2) Safe bridge to first IK point (slow + verified)
                    ...
                    # 3) Execute remaining path with adaptive timing (no fixed 0.2s jumps)
                    rest_path = continuous_points[1:]
                    if rest_path:
                        msg = JointTrajectory()
                        msg.joint_names = self.ur1_joints
                        msg.points = self._retime_joint_path(
                            rest_path, q_start=first, max_speed=0.30, min_dt=0.12
                        )
